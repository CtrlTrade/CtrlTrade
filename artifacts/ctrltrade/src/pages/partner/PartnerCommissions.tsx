  ...
  return (
    <div className="space-y-6">
      ...
      <Card className=" border-border">
        <CardHeader><CardTitle className="">Earnings</CardTitle></CardHeader>
        <CardContent>
          {!data || data.length === 0 ? (
              ...
              <tbody>
                {data.map((c) => (
                  <tr key={c.id} className="border-t border-border">
                    ...
                    <td className="py-2"><Badge variant={c.status === "paid" ? "default" : "secondary"} className="rounded-xl">{c.status}</Badge></td>
                  </tr>
                ))}
              </tbody>
            </table>
