  ...
  return (
    <div className="space-y-6">
      <Card className=" border-border">
        <CardHeader>
          <CardTitle className="">{editing ? "Edit campaign" : "New campaign"}</CardTitle>
          <CardDescription>Reward your existing customers for referring new ones.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2 md:col-span-2"><Label>Name</Label><Input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-xl" data-testid="input-campaign-name" /></div>
            ...
            <div className="space-y-2"><Label>{form.rewardType === "percent" ? "Percent (%)" : "Amount (pence)"}</Label><Input type="number" min={0} required value={form.rewardValuePence} onChange={(e) => setForm({ ...form, rewardValuePence: Number(e.target.value) })} className="rounded-xl" data-testid="input-campaign-value" /></div>
            ...
            <div className="space-y-2 md:col-span-2"><Label>Description</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="rounded-xl" /></div>
            <div className="md:col-span-2 flex gap-3">
              <Button type="submit" disabled={create.isPending || update.isPending} className="rounded-xl font-bold" data-testid="button-save-campaign">{editing ? "Update" : "Create"} campaign</Button>
              {editing && <Button type="button" variant="outline" className="rounded-xl" onClick={() => { setEditing(null); setForm(empty); }}>Cancel</Button>}
            </div>
          </form>
        </CardContent>
      </Card>
      ...
      <Card className=" border-border">
        <CardHeader><CardTitle className="">Active campaigns</CardTitle></CardHeader>
        <CardContent>
              ...
              <tbody>
                {campaigns.map((c) => (
                  <tr key={c.id} className="border-t border-border">
                    ...
                    <td className="py-2"><Badge variant={c.active ? "default" : "secondary"} className="rounded-xl">{c.active ? "Yes" : "No"}</Badge></td>
                    <td className="py-2 text-right space-x-2">
                      <Button type="button" variant="outline" size="sm" className="rounded-xl" onClick={() => { setEditing(c.id); setForm({ name: c.name, rewardType: c.rewardType as any, rewardValuePence: c.rewardValuePence, rewardForReferrer: c.rewardForReferrer, rewardForReferee: c.rewardForReferee, description: c.description ?? "", active: c.active }); }}>Edit</Button>
                      <Button type="button" variant="ghost" size="sm" className="rounded-xl text-destructive" onClick={() => del.mutate({ campaignId: c.id })}>Delete</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Card className=" border-border">
        <CardHeader><CardTitle className="">Referrals received</CardTitle></CardHeader>
        <CardContent>
              ...
              <tbody>
                {conversions.map((c) => (
                  <tr key={c.id} className="border-t border-border">
                    ...
                    <td className="py-2"><Badge variant={c.status === "rewarded" ? "default" : "secondary"} className="rounded-xl">{c.status}</Badge></td>
                    <td className="py-2 text-right">
                      {c.status !== "rewarded" && (
                        <Button type="button" variant="outline" size="sm" className="rounded-xl" onClick={() => reward.mutate({ conversionId: c.id })}>Issue reward</Button>
                      )}
                    </td>
                  </tr>
                ))}
