      ...
      <Card className="rounded-none border-border">
        ...
        <CardContent>
          ...
          >
            ...
            <Button type="submit" disabled={upsert.isPending} className="rounded-none uppercase font-bold tracking-wider" data-testid="button-flag-save">Save flag</Button>
          </form>
        </CardContent>
      </Card>
            ...
            <tbody>
              {(data ?? []).map((f) => (
                <tr key={f.id} className="border-b border-border" data-testid={`row-flag-${f.key}`}>
                  ...
                  <td><Input type="number" min={0} max={100} defaultValue={f.rolloutPct} className="rounded-none h-8 w-20" onBlur={(e) => { const v = Number(e.target.value); if (v !== f.rolloutPct) upsert.mutate({ data: { key: f.key, enabled: f.enabled, rolloutPct: v, tenantId: f.tenantId, description: f.description ?? undefined } }); }} /></td>
                  ...
                  <td className="text-right"><Button size="sm" variant="outline" className="rounded-none" onClick={() => { if (confirm(`Delete flag "${f.key}"?`)) del.mutate({ flagId: f.id }); }}><Trash2 className="h-3 w-3" /></Button></td>
                </tr>
              ))}
