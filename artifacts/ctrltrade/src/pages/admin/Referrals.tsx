              ...
              <tbody>
                {partners.map((p) => (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="py-2"><Badge variant={p.status === "approved" ? "default" : "secondary"} className="rounded-xl">{p.status}</Badge></td>
                    ...
                    <td className="py-2 text-right space-x-2">
                      {p.status !== "approved" && <Button size="sm" variant="outline" className="rounded-xl" onClick={() => update.mutate({ partnerId: p.id, data: { status: "approved" } })}>Approve</Button>}
                      {p.status !== "disabled" && <Button size="sm" variant="ghost" className="rounded-xl text-destructive" onClick={() => update.mutate({ partnerId: p.id, data: { status: "disabled" } })}>Disable</Button>}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody>
                {payouts.map((p) => (
                  <tr key={p.id} className="border-t border-border">
                    ...
                    <td className="py-2 w-48"><Input value={refs[p.id] ?? p.reference ?? ""} onChange={(e) => setRefs({ ...refs, [p.id]: e.target.value })} className="rounded-xl h-8" placeholder="Bank ref" /></td>
                    <td className="py-2 text-right space-x-2">
                      {p.status === "requested" && <Button size="sm" variant="outline" className="rounded-xl" onClick={() => decide.mutate({ payoutId: p.id, data: { status: "approved" } })}>Approve</Button>}
                      {p.status !== "paid" && p.status !== "rejected" && <Button size="sm" className="rounded-xl" onClick={() => decide.mutate({ payoutId: p.id, data: { status: "paid", reference: refs[p.id] } })}>Mark paid</Button>}
                      {p.status === "requested" && <Button size="sm" variant="ghost" className="rounded-xl text-destructive" onClick={() => decide.mutate({ payoutId: p.id, data: { status: "rejected" } })}>Reject</Button>}
                    </td>
                  </tr>
                ))}
              </tbody>
