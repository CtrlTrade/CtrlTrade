...
export function AdminLeadDetail() {
  ...
  if (!lead) {
    return (
      <div className="text-center py-20 text-zinc-600 font-mono">Lead not found.</div>
    );
  }
    ...
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-4">
        <Button
          variant="ghost"
          size="sm"
          className="gap-2 text-zinc-500 hover:text-zinc-200 uppercase text-xs font-bold"
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right">
                  <div className="text-xs text-zinc-600 font-mono">{new Date(lead.createdAt).toLocaleDateString()}</div>
                  <div className="text-xs text-zinc-500 uppercase mt-0.5">{lead.source}</div>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <a href={`mailto:${lead.email}`} className="text-zinc-300 hover:text-red-500 transition-colors font-mono text-xs">
                    {lead.email}
                  </a>
                </div>
                {lead.phone && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <a href={`tel:${lead.phone}`} className="text-zinc-300 hover:text-red-500 transition-colors font-mono text-xs">
                      {lead.phone}
                    </a>
                  </div>
                )}
                {lead.trade && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-zinc-400">{lead.trade}</span>
                  </div>
                )}
              </div>
            </CardContent>
            ...
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              ...
              {!notesOpen ? (
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-xs uppercase text-zinc-500 hover:text-zinc-200"
            ...
            <CardContent>
              {notesOpen ? (
                ...
                <p className="text-sm text-zinc-400 whitespace-pre-wrap font-mono">
                  {lead.notes || <span className="text-zinc-600 italic">No notes yet.</span>}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="rounded-none border-zinc-800 bg-black shadow-none">
            ...
            <CardContent>
              {(!lead.messages || lead.messages.length === 0) ? (
                <div className="py-6 text-center text-zinc-600 font-mono text-xs">No interactions yet.</div>
              ) : (
                <div className="space-y-3">
                  {lead.messages.map((msg: any) => (
                    <div key={msg.id} className="border-l-2 border-zinc-700 pl-3 py-1 space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold uppercase text-zinc-500">
                          {CHANNEL_LABELS[msg.channel] ?? msg.channel}
                        </span>
                        {msg.authorName && (
                          <span className="text-xs text-zinc-600">by {msg.authorName}</span>
                        )}
